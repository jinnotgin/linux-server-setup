...
`setup.sh` is an interactive bootstrap script for Ubuntu/Debian-like hosts. It focuses on SSH/root hardening, Docker/Portainer installation, daily backups to Google Drive via `rclone`, and optional rendering of ready-made Docker Compose templates (SSL renewal, Caddy reverse proxy, VLESS over WebSocket, and Hysteria2). You can run only the parts you want (e.g., skip hardening/install and just render templates).
...
- **ssl-renewal**: `certbot` standalone renewal that keeps certificates under `./certs`. Supports multiple domains. Binds port 80 only so it can coexist with Caddy on 443.
- **caddy**: reverse proxy that serves TLS using the certificates from `./certs` and proxies `/ws` to the Xray (VLESS) container over the shared `proxy_net` network.
- **vless-ws**: `teddysun/xray` serving VLESS over WebSocket without terminating TLS (Caddy does TLS on 443 and forwards to this service on port 10000 inside `proxy_net`). Supports multiple UUID clients.
...
- The Certbot template binds port 80; the Caddy template binds 443. They can co-exist without port conflicts, but ensure port 80 is free when running Certbot.
- Caddy terminates TLS using the certificates generated by Certbot; the VLESS/Xray service stays behind Caddy on `proxy_net` (no direct TLS on the Xray container).
- Before starting the Caddy or VLESS stacks, create the shared Docker network with `docker network create proxy_net` (the script will also create it automatically if Docker is available when you choose to auto-start stacks).
