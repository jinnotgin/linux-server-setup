...
- Portainer backups use `rclone config` with Google Drive OAuth and a remote named `portainer_gdrive`. For a personal Google Drive on a headless server, use browser-based OAuth from another computer when rclone asks for manual authorization; do not use a service account.
